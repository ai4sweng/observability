"""KIOEnvelope — vendored, byte-for-byte copy of orchestrator/envelope.py's
body (see that file for the full docstring/rationale). Duplicated here so
kio-simulator's Docker build context can stay `./kio-simulator` unchanged
(including for remote-kio, which reuses this directory as its build context
from outside the repo). Keep in sync when the schema changes.
