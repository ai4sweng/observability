...
| `metrics-api` | Parameterized read API over the metrics store — query telemetry without writing PromQL (see "Querying metrics over HTTP") | 8081 |
...
visible via `/api/metrics/catalog?family=kpi`.
...
CRUD tested via SQLite), and `metrics_api/` (the generated registry and its
...
```bash
uv venv
uv pip install -r kio-simulator/requirements.txt -r orchestrator/requirements.txt \
